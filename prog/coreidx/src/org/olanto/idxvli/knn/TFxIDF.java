 ...
 *
 */
public class TFxIDF implements KNNManager {
        ...
        for (int i = 0; i < docbag.length; i++) {  //pour chaque mot du docbag
            ...
            if (KNNFilter[wordinDB]) {
                ...
                for (int j = 0; j < lastwi; j++) { // pour chaque document commun de ce mot i
                    ...
                    else if (formulaTF == 2) { // 1
                        wtdid = 1;
                    } //
                    else if (formulaTF == 3) { // f(d,t)
                        wtdid = weightinDB;
                    } //

                    cumul[wi[wordinDB][j]] += wtd[wordinDB][j] * wtdid * wt[wordinDB];
    ...
    private final void computeWeightsTF(int formula) { // Term Frequency
            ...
            if (KNNFilter[i]) { // filter first
                ...
                for (int j = 0; j < lastwi; j++) {
                    ...
                    if (formula == 1) { // 1+ln(f(d,t))
                        wtd[i][j] = (1 + (float) Math.log((double) indexread.vDoc(i, j + lastwi)));
                    } //
                    if (formula == 2) { // 1
                        wtd[i][j] = 1;
                    } //
                    if (formula == 3) { // f(d,t)
                        wtd[i][j] = indexread.vDoc(i, j + lastwi);
                    } //
                    wd[wi[i][j]] += wtd[i][j] * wtd[i][j]; // sum(wtd^2)
                }
                // fin de zone
