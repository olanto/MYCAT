...
/**
 * gestionaire de mots géré sur disque avec des IO Map avec 2 hash (stocké dans
 * un long). <p>
 *
 *
 * <p>
 */
public class StringTable_HomeHash_InMemory_Clue_XL implements StringRepository {

    /* constantes d'un gestionnaire du dictionaire -------------------------------------- */
    static final String SOFT_VERSION = "StringTable_HomeHash_InMemory_Clue_XL 2.1";
    ...
    /**
     * crée une word table de taille 2^_maxsize par défaut à l'endroit indiqué
     * par le path
     * @param _pathName
     * @param _idxName
     * @param _maxSize
    ...
    private final void createStringTable_HomeHash_InMemory_Clue_XL(
        ...
        try {
            ...
            hdocclue = (new LongVector_InMemory()).create(pathName, idxName + ".cluedoc", maxSize);
            hdocclue = (new LongVector_InMemory()).open(pathName, idxName + ".cluedoc");
            initFirstTime();
            saveMasterFile();
        } catch (Exception e) {
            error("IO error in StringTable_HomeHash_InMemory_Clue_XL()", e);
        }
    ...
    /**
     * ajoute un terme au gestionnaire retourne le numéro du terme, retourne
     * EMPTY s'il y a une erreur, retourne son id s'il existe déja
     * @param w
     * @return 
     */
