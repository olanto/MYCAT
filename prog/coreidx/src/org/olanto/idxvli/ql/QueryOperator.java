...
import org.olanto.idxvli.cache.CacheRead_Opti;
import org.olanto.idxvli.cache.PosLength;
import org.olanto.idxvli.doc.DocumentManager;
...
//        System.out.println("debug Quotation:"+z);
//        showVector(tosearch);
...
//        System.out.println("debug in ql:");
//        showVector(res);
        return new QRes(res);
    }
...
//                        int ref2 = indexread.vPosDirect(cid2, pc2);
                        int ref1 = indexread.pos.v[cid1][pc1];
                        int ref2 = indexread.pos.v[cid2][pc2];
...
//            showVector(doc);
    ...
    public static final int[] getDocforWseqW3(IdxStructure z, int n1, int n2, int n3)  {
        ...
        int pc1, pc2, pl1, pl2;
        //  PosLength p1, p2;
        int p1_pos, p1_length, p2_pos, p2_length;
        if ((n1 != NOT_FOUND) & (n2 != NOT_FOUND) & (n3 != NOT_FOUND) ) {
            ...
            while (true) { // merge sort  r1 and r2 must be ordered !!!!!
                ...
                if (r1[wc1] == r2[wc2]) { // and ok
                    //   p1 = indexread.getWPosLengthDirect(cid1, wc1);
                    int tl = indexread.nbDoc[cid1];
                    p1_pos = indexread.doc.v[cid1][ tl * z.VEC + wc1];
                    p1_length = indexread.doc.v[cid1][ tl * z.OCC + wc1];
                    //   p2 = indexread.getWPosLengthDirect(cid2, wc2);
                    int t2 = indexread.nbDoc[cid2];
                    p2_pos = indexread.doc.v[cid2][ t2 * z.VEC + wc2];
                    p2_length = indexread.doc.v[cid2][ t2 * z.OCC + wc2];
                    pl1 = p1_pos + p1_length;
                    pl2 = p2_pos + p2_length;
                    pc1 = p1_pos;
                    pc2 = p2_pos;
                    while (true) { // initialize a new merge sort  r1 and r2 must be ordered !!!!!
                        if (pc1 >= pl1 || pc2 >= pl2) {
...
//                        int ref2 = indexread.vPosDirect(cid2, pc2);
                        ...
                        int ref2 = indexread.pos.v[cid2][pc2];
                        // System.out.println("  merge2 "+pc1+", "+r1[pc1]+", "+pc2+", "+r2[pc2]);
                        if (((ref2 - ref1) == 1) && (indexread.chekforWatPinDDirect(cid3, ref2 + 1, r1[wc1])) ) { // near ok and test third, ...
                            doc[id] = r1[wc1]; // the first operand stay the reference for occurrences
                            id++;
                            break;
                        } else if (ref1 < ref2) {
