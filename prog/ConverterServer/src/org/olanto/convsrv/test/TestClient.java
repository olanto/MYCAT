/**
 * ********
 * Copyright © 2010-2012 Olanto Foundation Geneva
 *
 * This file is part of myCAT.
 *
 * myCAT is free software: you can redistribute it and/or modify it under the
 * terms of the GNU Affero General Public License as published by the Free
 * Software Foundation, either version 3 of the License, or (at your option) any
 * later version.
 *
 * myCAT is distributed in the hope that it will be useful, but WITHOUT ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR
 * A PARTICULAR PURPOSE. See the GNU Affero General Public License for more
 * details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with myCAT. If not, see <http://www.gnu.org/licenses/>.
 *
 *********
 */
package org.olanto.convsrv.test;

import java.rmi.Naming;
import java.rmi.Remote;
import org.olanto.convsrv.server.ConvertService;

/**
 * Test du client de conversion
 *
 *
 */
public class TestClient {

    public static void main(String[] args) {


        try {

            System.out.println("connect to server");

            Remote r = Naming.lookup("rmi://localhost/CONVSRV");

            System.out.println("access to server");

            if (r instanceof ConvertService) {
                ConvertService is = ((ConvertService) r);
                String s = is.getInformation();
                System.out.println("receive = " + s);

                byte[] bytes = null;
           String fileName ="C:\\MYCAT\\corpus\\docs\\small-collection\\UNO\\A_RES_53_144_EN.pdf";
//              String fileName ="C:\\MYCAT\\corpus\\docs\\small-collection\\UNO\\le-petit-prince1_FR.txt";

                String ret = is.File2Txt(fileName);

                System.out.println("Converted file content: " + ret);


                System.out.println("end ...");


            }

        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
